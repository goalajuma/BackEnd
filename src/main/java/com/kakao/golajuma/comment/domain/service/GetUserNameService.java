...
public class GetUserNameService {
	...
	private void putUsernameMapFromEntities(List<CommentEntity> commentEntities){
		VoteEntity voteEntity = voteRepository.findById(commentEntities.get(0).getVoteId()).orElseThrow(NotFoundVoteException::new);
		for (CommentEntity commentEntity : commentEntities) {
			String userNickname = getUserNickname(commentEntity.getUserId());
			usernameMap.computeIfAbsent(commentEntity.getUserId(), k -> new String[]{userNickname, null});
