...
public class GetCommentsService {
	...
	private List<ParentCommentDto> commentDtosConverter(List<CommentEntity> commentEntities, Long userId) {
		Map<Long,String[]> usernameMap = getUserNameService.execute(commentEntities);

		Map<Boolean, List<CommentEntity>> partitionedEntities = commentEntities.stream()
				.collect(Collectors.partitioningBy(CommentEntity::isParent));
		...
		List<ParentCommentDto> parentCommentDtos = parentEntities.stream()
		...
		for (CommentEntity childEntity: childEntities){
			parentCommentDtos.stream()
					.filter(parentCommentDto -> Objects.equals(parentCommentDto.getId(), childEntity.getParentId()) && !childEntity.isDeleted())
					.findFirst()
					.ifPresent(parentCommentDto -> parentCommentDto.addReply(toChildCommentDto(userId, childEntity, usernameMap)));
		}

		parentCommentDtos.removeIf(parentCommentDto -> Objects.equals(parentCommentDto.getContent(), DELETED_COMMENT_CONTENT) && parentCommentDto.getReply().isEmpty());
	...
	private CommentDto toCommentDto(Long userId, CommentEntity commentEntity, Map<Long, String[]> usernameMap, boolean isParent) {
		...
		if (isParent) {
			if (Boolean.TRUE.equals(commentEntity.isDeleted())) commentEntity.updateContent(DELETED_COMMENT_CONTENT);
	...
	public CommentDto toChildCommentDto(Long userId, CommentEntity commentEntity, Map<Long, String[]> usernameMap) {
		return toCommentDto(userId, commentEntity, usernameMap, false);
	}

	public ParentCommentDto toParentCommentDto(Long userId, CommentEntity commentEntity, Map<Long, String[]> usernameMap) {
		return (ParentCommentDto) toCommentDto(userId, commentEntity, usernameMap, Boolean.TRUE.equals(commentEntity.isParent()));
	}
}
